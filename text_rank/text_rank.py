...
def calculate_similarity_matrix(vectors: np.ndarray) -> np.ndarray:
    """Calculates the cosine similarity matrix between vectors."""
    similarity_matrix = cosine_similarity(vectors)
    # Set diagonal to 0 (self-similarity)
    np.fill_diagonal(similarity_matrix, 0)
    return similarity_matrix

def visualise_similarity_graph(similarity_matrix, sentences, threshold=0.3, title="Sentence Similarity Graph", save_path=None):
    """Visualizes the similarity graph using NetworkX and Matplotlib."""
...
def apply_textrank(similarity_matrix: np.ndarray) -> dict:
    """Applies the TextRank algorithm to the similarity matrix and returns the scores."""
    # Create a graph from the similarity matrix
...
def get_relevant_sentences(sentences, scores, num_sentences) -> list:
    """Selects the sentences with the highest scores to form the most relevant sentences."""
    # Sort the sentences by their scores in descending order
    ranked_sentences = sorted(((scores[i], i, sentence) for i, sentence in enumerate(sentences)), reverse=True)
    # Select the top N sentences
    relevant_sentences = [(i+1, idx+1, score, sentence) for i, (score, idx, sentence) in enumerate(ranked_sentences[:num_sentences])]
...
if __name__ == "__main__":
    ...
    print("\nProcessing text and generating representations...")
    ...
    print("Running TextRank algorithm on TF-IDF matrix...")
    ...
    print("Visualizing embedding similarity graph...")
    ...
    print("Running TextRank algorithm on embedding matrix...")
    ...
    tfidf_relevant_sentences = get_relevant_sentences(sentences, tfidf_scores, num_relevant_sentences)
    ...
    embedding_relevant_sentences = get_relevant_sentences(sentences, embedding_scores, num_relevant_sentences)
