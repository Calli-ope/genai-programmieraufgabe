...
def create_tokenizer_charts(
    german_metrics, english_metrics, combined_metrics, test_sets
):
    tokenizers = {
...
def main():
    de_tokenizer = BPETokenizer.load(
        f"{TOKENIZERS_DIR}german_vocab{VOCAB_SIZE}_tokenizer.pkl"
    )
    en_tokenizer = BPETokenizer.load(
        f"{TOKENIZERS_DIR}english_vocab{VOCAB_SIZE}_tokenizer.pkl"
    )
    combined_tokenizer = BPETokenizer.load(
        f"{TOKENIZERS_DIR}combined_vocab{VOCAB_SIZE}_tokenizer.pkl"
    )
    ...
    for test_set_name, sentences in test_sets.items():
        ...
        for tokenizer_name, tokenizer in tokenizers.items():
            ...
            if tokenizer_name == "German":
                german_metrics[test_set_name] = calculate_efficiency(
                    tokenizer, sentences
                )
            elif tokenizer_name == "English":
                english_metrics[test_set_name] = calculate_efficiency(
                    tokenizer, sentences
                )
            else:
                combined_metrics[test_set_name] = calculate_efficiency(
                    tokenizer, sentences
                )

    create_tokenizer_charts(
        german_metrics, english_metrics, combined_metrics, test_sets
    )
